  ...
  return (
    <nav className="sticky top-0 z-50 w-full glass-panel border-b border-accent/10 py-4 px-6 md:px-12 flex justify-between items-center transition-all duration-300">
      ...
      {isOpen && (
        <div
          id="mobile-menu"
          className="absolute top-16 left-0 right-0 glass-panel border-b border-accent/10 py-4 flex flex-col items-center space-y-4 md:hidden animate-fade-in-down"
        >
          {navItems.map((item) => {
            const isActive = pathname === item.path;
            ...
            if (item.disabled) {
              return (
                <span
                  key={item.path}
                  className="text-textMuted/40 font-body text-base font-medium cursor-not-allowed py-2"
                >
                  {item.name} (Calculated first)
                </span>
              );
            }
            ...
            return (
              <Link
                ...
                className={`font-body text-base font-medium transition-all duration-200 w-full text-center py-2 focus:outline-none focus:ring-2 focus:ring-accent ${
